...
var DialogueScreen = Screen.extend(function(id, file){
  ...
  this.topDialogue = new Sprite('topDialogue', 0, 0, 'Sprites/Dialogue/top_dialogue.png', 1);
  this.bottomDialogue = new Sprite('bottomDialogue', 0, 0, 'Sprites/Dialogue/bottom_dialogue.png', 1);
  this.topDialogue.left = Math.floor((g.origins.width - this.topDialogue.width) / 2);
  this.bottomDialogue.left = Math.floor((g.origins.width - this.bottomDialogue.width) / 2);
  this.bottomDialogue.top = g.origins.height - this.bottomDialogue.height;
  this.spriteArray.push(this.topDialogue, this.bottomDialogue);
    ...
    draw: function(HTML){
      if(this.id == g.activeDialogue){
        ...
        if (this.activeStatement instanceof OverseerStatement){
          this.overseerHTML = this.activeStatement.draw();
          if (this.activeStatement.nextType === 'overseer'){
            this.playerHTML = '<div>Click to continue</div>';
          } else if (this.activeStatement.nextType === 'exit'){
            this.playerHTML = '<div>Click to exit</div>';
          }
        } else if (this.activeStatement instanceof PlayerStatement or this.activeStatement instanceof PopupStatement){
          this.playerHTML = this.activeStatement.draw();
        }
        HTML.push(this.overseerHTML, this.playerHTML, ' </div>');
        */
        HTML.push('</div>');
      }
    }
...
var Statement = klass(function(parent, xmlData){
  ...
  this.textBlocks.push(new xmlTextBox(xmlData, 0, 0, 1000, 300, '20px'));  
})
  .methods({
