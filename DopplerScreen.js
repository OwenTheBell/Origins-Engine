...
var Emitter = klass(function(x, y, radius, pulsePerSecond, speed){
  ...
  .methods({
    update: function() {
      ...
      if ((this.centerX + this.radius) > (doppler.canvas.width - 20)){
        this.centerX = doppler.canvas.width - this.radius - 20;
      } else if ((this.centerX - this.radius) < 20) this.centerX = this.radius + 20;
      if ((this.centerY + this.radius) > (doppler.canvas.height - 120)){
        this.centerY = doppler.canvas.height - this.radius - 120;
